...
func (d *Daemon) runLoop(ctx context.Context, done chan struct{}) {
	...
	var pullTickCh <-chan time.Time
	if d.workSourceEnabled {
		pullTick := d.newTicker(d.workSourcePullInterval)
		defer pullTick.Stop()
		pullTickCh = pullTick.C()
	}
	...
	tickDone := make(chan monitorTickResult, 4)
...
func (d *Daemon) runPollTick(ctx context.Context) *pollTickTiming {
	...
	if err == nil {
		...
		done = timing.stage("terminal_state_batch")
		ctx = d.withBatchedPRTerminalStates(ctx, assignments)
		done()
		done = timing.stage("monitor_checks")
