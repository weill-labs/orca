...
func (s *PostgresStore) UpsertDaemon(ctx context.Context, project string, daemon DaemonStatus) error {
	...
	`, daemon.Host, project, daemon.Session, daemon.PID, daemon.Status, normalizeTime(daemon.StartedAt), normalizeTime(daemon.UpdatedAt))
	if err != nil {
...
func (s *PostgresStore) MarkDaemonStopped(ctx context.Context, project string, updatedAt time.Time) error {
	...
	_, err := s.pool.Exec(ctx, `
		INSERT INTO daemon_statuses(host, project, session, pid, status, started_at, updated_at)
		VALUES($1, $2, '', 0, 'stopped', $3, $4)
		ON CONFLICT(host, project) DO UPDATE SET
			pid = 0,
			status = 'stopped',
			updated_at = excluded.updated_at
	`, s.host, project, normalizeTime(updatedAt), normalizeTime(updatedAt))
	if err != nil {
...
func (s *PostgresStore) ClaimWorker(ctx context.Context, project string, worker Worker) (Worker, error) {
	...
	row := tx.QueryRow(ctx, `
		...
		FROM workers
		WHERE project = $1 AND agent_profile = $2 AND issue = '' AND `+postgresHostMatch("host", 3)+`
	...
	case scanErr == nil:
		...
		if _, err := tx.Exec(ctx, `
			UPDATE workers
			SET host = $1, issue = $2, state = $3, last_seen_at = $4
			WHERE project = $5 AND worker_id = $6
		`, s.host, claimed.Issue, claimed.State, normalizeTime(claimed.LastSeenAt), project, claimed.WorkerID); err != nil {
	...
	case errors.Is(scanErr, pgx.ErrNoRows):
		...
		if _, err := tx.Exec(ctx, `
			INSERT INTO workers(project, worker_id, host, agent_profile, current_pane_id, state, issue, clone_path, last_review_count, last_inline_review_comment_count, last_issue_comment_count, last_issue_comment_watermark, last_review_updated_at, review_nudge_count, review_approved, last_ci_state, ci_nudge_count, ci_failure_poll_count, ci_escalated, last_mergeable_state, nudge_count, last_capture, last_activity_at, last_pr_number, last_push_at, last_pr_poll_at, restart_count, first_crash_at, created_at, last_seen_at)
			VALUES($1, $2, $3, $4, '', $5, $6, '', 0, 0, 0, '', NULL, 0, FALSE, '', 0, 0, FALSE, '', 0, '', NULL, 0, NULL, NULL, 0, NULL, $7, $8)
		`, project, worker.WorkerID, s.host, worker.Agent, worker.State, worker.Issue, normalizeTime(worker.CreatedAt), normalizeTime(worker.LastSeenAt)); err != nil {
...
func (s *PostgresStore) DeleteTask(ctx context.Context, project, issue string) error {
	tag, err := s.pool.Exec(ctx, `
		DELETE FROM tasks
		WHERE project = $1 AND issue = $2 AND `+postgresHostMatch("host", 3)+`
...
func (s *PostgresStore) EnsureClone(ctx context.Context, project, path string) (legacy.CloneRecord, error) {
	...
	`, project, path, s.host, normalizeTime(now)); err != nil {
...
func (s *PostgresStore) TryOccupyClone(ctx context.Context, project, path, branch, task string) (bool, error) {
	tag, err := s.pool.Exec(ctx, `
		UPDATE clones
		SET host = $1, status = 'occupied', issue = $2, branch = $3, updated_at = $4
		WHERE project = $5 AND path = $6 AND status = 'free' AND `+postgresHostMatch("host", 7)+`
	`, s.host, task, branch, normalizeTime(s.now()), project, path, s.host)
	if err != nil {
...
func (s *PostgresStore) RecordCloneFailure(ctx context.Context, project, path string, threshold int) (legacy.CloneRecord, error) {
	if threshold <= 0 {
		threshold = 1
	}
	tag, err := s.pool.Exec(ctx, `
		...
		SET host = $1,
		    ...
		    status = CASE WHEN failure_count + 1 >= $2 THEN 'quarantined' ELSE status END,
...
func (s *PostgresStore) ResetCloneFailures(ctx context.Context, project, path string) error {
	tag, err := s.pool.Exec(ctx, `
		UPDATE clones
		SET host = $1, failure_count = 0, updated_at = $2
...
func (s *PostgresStore) MarkCloneFree(ctx context.Context, project, path string) error {
	tag, err := s.pool.Exec(ctx, `
		UPDATE clones
		SET host = $1, status = 'free', issue = '', branch = '', updated_at = $2
		WHERE project = $3 AND path = $4 AND `+postgresHostMatch("host", 5)+`
	`, s.host, normalizeTime(s.now()), project, path, s.host)
	if err != nil {
...
func (s *PostgresStore) UpdateTaskStatus(ctx context.Context, project, issue, status string, updatedAt time.Time) (Task, error) {
	...
	tag, err := s.pool.Exec(ctx, `
		UPDATE tasks
		SET host = $1, status = $2, state = CASE WHEN $2 IN ('done', 'cancelled', 'failed') THEN 'done' ELSE state END, updated_at = $3
		WHERE project = $4 AND issue = $5 AND `+postgresHostMatch("host", 6)+`
	`, s.host, status, normalizeTime(updatedAt), project, issue, s.host)
	if err != nil {
