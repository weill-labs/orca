...
func (d *Daemon) startAssignmentWorker(ctx context.Context, projectPath string, clone Clone, task Task, worker Worker, profile AgentProfile, prompt, paneTitle string, preflightSkipped bool, planningDecision string) (assignStartupResult, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		metadata, err := d.assignmentPaneMetadata(ctx, projectPath, pane.ID, profile.Name, attemptTask.Branch, attemptTask.Issue, paneTitle, attemptTask.PRNumber, preflightSkipped, planningDecision)
		if err != nil {
