...
func TestPRPollFallbackCompletesTaskFromPersistedSQLiteState(t *testing.T) {
	...
	deps.commands.queue("gh", []string{"pr", "list", "--search", "LAB-1293 in:title", "--state", "all", "--json", "number,state,headRefName", "--limit", "5"}, `[{"number":166,"state":"MERGED","headRefName":"lab-1293-mergeable-unknown"}]`, nil)
